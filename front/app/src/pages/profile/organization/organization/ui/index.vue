...
const addUserSchema = z.object({
  userId: z.string().min(1, validationErrors.minChars(1)).max(128, validationErrors.maxChars(128)),
})

const { defineField: defineAddUserField, meta: addUserMeta, handleSubmit: handleAddUserSubmit } = useForm({ validationSchema: toTypedSchema(addUserSchema) })
const [userId, userIdAttrs] = defineAddUserField('userId')
const onAddUserSubmit = handleAddUserSubmit((values) => {
  console.log('add user', values.userId)
})
  ...
  >
    ...
    <section>
      ...
      <Dialog>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <VisuallyHidden><DialogDescription>Добавить сотрудника</DialogDescription></VisuallyHidden>
          </DialogHeader>
          <form
            class="flex flex-col gap-y-4"
            @submit.prevent
          >
            ...
            <Button
              :disabled="!(addUserMeta.dirty && addUserMeta.valid)"
              type="submit"
            >
              Добавить
            </Button>
      ...
      <div class="mt-4 grid grid-cols-[repeat(auto-fill,minmax(260px,1fr))] gap-x-8 gap-y-5">
        <EmployeeCard
          v-for="employee in organization.employees"
          :key="employee.id"
          :employee="employee"
          :has-recommendation-from-me="false"
        />
      </div>
    </section>
