...
{
    public class OrganizationRepository : IUserRepository
    {
        ...
        {
            return await _context.Organizations.FirstOrDefaultAsync(u => u.Id == id);
        ...
        public async Task<bool> UserExist(string login)
        {
            try
            {
                return await _context.Organizations.AnyAsync(u => u.Email == login);
            }
            catch (Exception ex)
            {
